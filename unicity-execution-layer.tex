...
\textit{Informally:} An adversary observing a transaction cannot determine which of two candidate recipients it was sent to, beyond random guessing. The transaction-specific public key $P_{tx}$ reveals no information about the underlying persistent address $P$.
...
\textit{Informally:} The token ownerhship-related fields in each transaction appears as uniformly random data. The pair $(P_{tx}, R)$ in any transaction is computationally indistinguishable from two random group elements, making it impossible to correlate transactions based on their public components.
...
\textit{Informally:} An adversary observing multiple transactions cannot determine which transactions belong to the same recipient. Even when a recipient reuses their persistent public key $P$ across multiple transactions, the resulting transaction-specific keys $P_{tx,i}$ and $P_{tx,j}$ appear unrelated.
