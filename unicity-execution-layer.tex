...
\subsubsection{Mask Derivation via Binding Signature}

We observe that deterministic signature schemes (e.g., EdDSA) produce a unique signature for any given message and private key. This property enables verifiable, deterministic mask derivation without revealing the private key.

\paragraph{Binding Signature Construction}

The recipient $B$ computes a \emph{binding signature} on the incoming state hash:
\[
\sigma_{\mathsf{bind}} \gets \sig(\prikey', \sthash)
\]

The mask is derived as:
\[
y \gets H(\sigma_{\mathsf{bind}})
\]

Since the signature scheme is deterministic, $\sigma_{\mathsf{bind}}$ is uniquely determined by $(\prikey', \sthash)$, and hence $y$ is uniquely determined by the incoming transaction state.
...
\begin{enumerate}
\item Binding signature: $\sigma_{\mathsf{bind}} \gets \sig(\prikey', \sthash)$
\item Mask: $y \gets H(\sigma_{\mathsf{bind}})$
...
\item Request to $\unisrv$: $Q' = (\pubkey', \sthash', \txhash', \sigma')$
\item Obtain inclusion proof $\pinc'$
\item Send to $C$: $(T', \sigma', \txhash', d', \pinc'; \sigma_{\mathsf{bind}})$ along with ledger $\mathcal{L}$
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Verify binding signature}:
\[
\sigver(\pubkey', \sthash, \sigma_{\mathsf{bind}}) \stackrel{?}{=} 1
\]

\item \textbf{Compute mask}:
\[
y \gets H(\sigma_{\mathsf{bind}})
\]
...
\begin{proof}
Given a certified transaction in state $(\pubkey, \sthash)$ transferring to $\pubkey'$, the binding signature $\sigma_{\mathsf{bind}} = \sig(\prikey', \sthash)$ is uniquely determined by $\sthash$ and the private key $\prikey'$ corresponding to $\pubkey'$.

For a deterministic signature scheme, there exists exactly one valid signature on message $\sthash$ under key $\prikey'$. Therefore $y = H(\sigma_{\mathsf{bind}})$ is uniquely determined, and consequently $\sthash' = H(\sthash, y)$ is unique.

To produce a different valid state $\sthash'' \neq \sthash'$, party $B$ would need a different valid signature $\sigma'_{\mathsf{bind}}$ satisfying $\sigver(\pubkey', \sthash, \sigma'_{\mathsf{bind}}) = 1$, contradicting determinism.
\end{proof}

\begin{theorem}[Timing Privacy]
If the signature scheme is unforgeable, party $A$ cannot determine when $B$ spends the token.
\end{theorem}

\begin{proof}
To compute the registry key $H(\pubkey', \sthash')$ that $\unisrv$ will record when $B$ spends, party $A$ must compute $\sthash' = H(\sthash, y)$ where $y = H(\sigma_{\mathsf{bind}})$.

Party $A$ knows $\pubkey'$ and $\sthash$. However, computing $\sigma_{\mathsf{bind}} = \sig(\prikey', \sthash)$ requires $\prikey'$.

Since $A$ does not possess $\prikey'$, producing a valid $\sigma_{\mathsf{bind}}$ constitutes an existential forgery, contradicting the unforgeability of the signature scheme.
\end{proof}
...
Computing $y = H(\sigma_{\mathsf{bind}})$ requires the binding signature $\sigma_{\mathsf{bind}} = \sig(\prikey', \sthash)$. The service knows $\sthash$ but not $\prikey'$, and therefore cannot compute $\sigma_{\mathsf{bind}}$.

Without $\sigma_{\mathsf{bind}}$, the service cannot compute $y$ or verify any relationship between $\sthash$ and $\sthash'$.
\end{proof}

\paragraph{Implementation Considerations}

This protocol requires the mask derivation signature scheme to be deterministic, meaning $\sig(\prikey, m)$ produces the same output for fixed $(\prikey, m)$. Standard choices include:
\begin{itemize}
\item ECDSA with RFC 6979 deterministic $k$ generation
\item EdDSA (inherently deterministic)
\item Schnorr signatures with deterministic nonce derivation (BIP-340)
\end{itemize}
...
\item \textbf{Binding signature}: $\sigma_{\mathsf{bind}} \gets \sig(d_{\mathsf{tx}}, \sthash)$

\item \textbf{Mask derivation}: $y \gets H(\sigma_{\mathsf{bind}})$

\item \textbf{Next state}: $\sthash' \gets H(\sthash, y)$

\item \textbf{Store}: $(d_{\mathsf{tx}}, \sthash', \sigma_{\mathsf{bind}})$ for spending
\end{enumerate}
...
\item \textbf{Send to $C$}: $(T', \sigma', \txhash', d', \pinc'; \sigma_{\mathsf{bind}})$ along with ledger $\mathcal{L}$
\end{enumerate}
...
\item \textbf{Verify binding signature}:
\[
\sigver(P_{\mathsf{tx}}, \sthash_{\mathsf{prev}}, \sigma_{\mathsf{bind}}) \stackrel{?}{=} 1
\]
