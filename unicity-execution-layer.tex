...
The contributions of this paper include: (1) a formal security model for off-chain transactions with on-chain double-spending prevention, modeled as trusted service in this paper's scope, (2) cryptographic protocols ensuring transaction privacy and preventing attacks of blocking token spending, (3) formal proofs of these security properties.
...
Unicity as a transacting framework provides three essential guarantees: (1) \emph{unique spending}—a digital asset can be spent at maximum once, (2) \emph{non-blocking}—only the legitimate owner of an asset can mark this asset as spent, and (3) \emph{privacy}—transaction details remain confidential between participants, including all subsequent owners of a token and also the Unicity Service.
...
To prevent information leakage, the transaction data $D$ is committed using a perfectly hiding commitment scheme, producing a transaction hash $\txhash = \commitc(H(D))$. The sender signs $H(\sthash, \txhash)$ and submits a certification request $Q = (\pubkey, \sthash, \txhash, \sigma)$ to the Unicity Service.

\subsubsection{Double-Spending Prevention}

The Unicity Service processes certification requests by checking: (1) the digital signature is valid, and (2) the key $H(\pubkey, \sthash)$ has not been previously registered. If both conditions hold, the service records the mapping $R[H(\pubkey, \sthash)] \gets \txhash$ and returns an inclusion proof $\pinc$. This mechanism ensures that each token state can be spent exactly once.
...
\end{enumerate}
The set $\Omega$ is called the \emph{sample set} and $\prob$ is called the \emph{probability function}. The subsets of $\Omega$ are called \emph{events}. For the probability $\prob[\{\omega\}]$ of a singleton subset we use shorthand notation $\prob[\omega]$.
...
A \emph{cryptographic primitive} is described as a list of (parametrized) algorithms, correctness conditions (invariants) and attacking scenarios. Adversaries are algorithms that participate in the security scenarios (interacting with environment) and break (are successful in the attacking scenario of) the primitive with certain success probability. If the parameters of a cryptographic primitive are fixed, we get an \emph{instance} of the primitive.
...
A signature scheme $(\keygen, \sig, \sigver)$ is $S$-secure against existential forgeries under adaptive chosen message attacks ($S$-secure EF-CMA) if it has $S$ as a security profile in the attack scenario:
\begin{enumerate}
...
A function $f$ is $S$-secure one-way if it has $S$ as a security profile in the attack scenario:
\begin{enumerate}
...
A hash function family $(\hfgen,\hfunc)$ is $S$-secure collision-resistant if it has $S$ as a security profile in the attack scenario:
\begin{enumerate}
...
\end{enumerate}
\end{definition}
In the following, we assume that the sampling $\param \gets \hfgen$ has been done before any attack scenario, and we often say that the function $H=\hfunc(\param; \cdot)$ itself is collision-resistant regardless of the fact that no fixed function can formally be collision-resistant.

\begin{definition}[$(k,\ell)$-One-Wayness]
A function $H\colon \{0,1\}^\ast\rightarrow\{0,1\}^k$
is $S$-secure $(k,\ell)-$\emph{one-way} if it has $S$ as a security profile in the attack scenario:
\begin{enumerate}
\item $(h,a)\gets A_1$
\item $x \gets \{0,1\}^\ell$
...
\end{enumerate}
\end{definition}
Equivalently, $H$ is $S$-secure $(k,\ell)$-one-way iff the function $f_h$ defined by $f_h(x)=H(h,x)$ is $S$-secure one-way for every $h\in\{0,1\}^k$.
...
A commitment scheme $(\setup, \commit, \open)$ is $S$-secure computationally binding if it has $S$ as a security profile in the attack scenario:
\begin{enumerate}
...
If $(\setup, \commit, \open)$ is perfectly hiding commitment scheme, $m$ is chosen according to any probability distribution and $g$ be any deterministic function, then $m$ and $\commitc(g(m))$ are independent random variables.
\end{lemma}
\begin{proof}
Let $\omega \gets \Omega$ be the internal randomness sampling of $\commitc$. We denote by $\commitc_\omega$ the deterministic version of $\commitc$, when the internal random string $\omega$ is fixed. Let $\mathcal{M}$ denote the sampling space of $m$. We assume that the sampling $\mu\gets \mathcal{M}$ happens independently of $\omega \gets \Omega$ and hence, the total sampling space is a direct product space with sampling space $\mathcal{M}\times \Omega$ and $\prob[\mu,\omega]=\prob[\mu]\cdot \prob[\omega]$ for any values $\mu, \omega$.

Let $m=M(\mu,\omega)$ with $\mu\gets \mathcal{M}$ and $\omega\gets \Omega$, i.e. $M$ is the random variable corresponding to $m$. Let $C$ be the corresponding random variable of $c$, i.e. $c=C(\mu,\omega)=\commitc_\omega(g(M(\mu,\omega)))$. Note also that $M(\mu,\omega)$ does not depend on $\omega$.
...
\end{lemma}
\begin{proof}
Let $f$ be an $S$-secure one-way function and $A$ be a $t$-time adversary that with probability $\epsilon$ succeeds in the attacking scenario.
...
\item The attack is successful if $f(x')=f(x)$.
\end{enumerate}
From Lemma~\ref{le:outputindependence} it follows that in both input distributions $(f(x),\commit(x))$ and $(f(x),\commit(x''))$ the commitments are independent of $x$ and are equally distributed, and hence
...
\item Return $A(y,c)$
\end{enumerate}
The adversary $A'$ inverts $f$ with probability $\epsilon$ and has running time $t+t_\mathsf{sm} + t_\mathsf{com}$. Therefore, $t+t_\mathsf{sm} + t_\mathsf{com}\ge S_f(\epsilon)$ and hence $t\ge S_f(\epsilon)- t_\mathsf{sm} - t_\mathsf{com}$.
\end{proof}
\medskip
...
We assume that transferred tokens can be sent using any channels and their storage does not require dedicated hardware devices. At the same time, the infrastructure has to guarantee some properties of the tokens such as \emph{unique ownership}, i.e. the owner of a token should not be able to transfer the token to two different parties (i.e. \emph{double-spend} the token) and once the token has been transferred, neither the previous owner nor any third parties should be able to do anything with the token -- transfer it or making it unusable to the next owner (i.e. \emph{blocking} the token).

As nothing prevents copying digital information, some additional components are needed in the infrastructure to guarantee the desired properties of tokens. Therefore, Unicity infrastructure includes \emph{Unicity service} -- an online functionality that all parties can communicate with and certify
...
\emph{Unicity service} $\unisrv$ is modeled as a state machine with state $R$, which is a key-value store (dictionary), where both keys and values are of type $\bits{|k|}$. Initially, $R = \emptyset$. We will write $R[k] = \bot$ if there are no pairs $(k, v)$ stored in $R$.

Every input request $Q$ is a tuple $(\pubkey, \sthash, \txhash, \sigma)$, where:
\begin{itemize}
\item $\pubkey$ of type $\bits{p}$ is a public key (public key of the current owner of a token, i.e. the owner before the transaction with the hash $\txhash$ is executed),
\item $\sthash$ of type $\bits{k}$ is a state hash (hash of the state of the token before the transaction is executed),
...
The request $Q=(\pubkey,\sthash,\txhash,\sigma)$ is processed by $\unisrv$ with the state $R$ as follows:
\begin{enumerate}
...
It is easy to see that if $R_0 = \emptyset$ is the initial state, $Q_1, Q_2, \ldots, Q_n$ is any sequence of queries, and $R_i$ is the state after the request $Q_i$ then:
\begin{itemize}
...
\item A hash value $h_T = H(\sthash, \txhash)$ is computed.
\item A digital signature $\sigma$ on $h_T$ is created with the private counterpart of $\pubkey$, i.e. $\mathsf{V}(\pubkey, h_T, \sigma) = 1$.
%\item A \emph{signed transaction} $(T,\sigma,d)$ is formed.
\item Create a query $Q\gets (\pubkey,\sthash,\txhash,\sigma)$
\item Call $\unisrv$ to obtain $\pi\gets \unisrv(Q)$
\item Form the certified transaction $(T,\sigma,\txhash,d,\pi)$.
\end{enumerate}
...
Mint transaction is the first transaction with every token. Mint transaction assigns an unique \emph{Token Identifier} $\mathsf{id}$ and some more application-specific data fields, like a \emph{Mint Justification}, packed into the auxiliary data $\auxd$.

A certified mint transaction is $(T_0, \sigma_0, \pi_0)$, where $T_0 = (\sthash, D_\mathsf{mint})$ and $D_\mathsf{mint}$ contain the following fields:
\begin{itemize}
    \item $\pubkey'$ -- the public key of the first owner, not masked
    \item $\mathsf{id}$ -- token identifier
% \item $x$ -- a uniformly chosen random string $x\gets\{0,1\}^\ell$
    \item $\auxd$ -- other data of the transaction
\end{itemize}

\noindent Mint transaction is signed by a fixed, public keypair $(\pubkey_\mathsf{mint}, \prikey_\mathsf{mint})$. This ensures the uniqueness of every token identifier. $\mathsf{MINT\_SUFFIX}$ is a fixed domain separator.\medskip

\noindent\textbf{Certifying a mint transaction} $T = (\sthash, D_\mathsf{mint})$ involves the following steps:
\begin{enumerate}
    \item $\sthash \gets H(\mathsf{id}, \mathsf{MINT\_SUFFIX})$
	\item $\txhash \gets H(D_\mathsf{mint})$ is computed.
	\item A hash value $h_T = H(\sthash, \txhash)$ is computed.
	\item A digital signature $\sigma$ on $h_T$ is created with the (publicly known, fixed) private key $\prikey_\mathsf{mint}$.
	\item Create a request $Q\gets (\pubkey_\mathsf{mint},\sthash,\txhash,\sigma)$
	\item Call $\unisrv$ to obtain $\pi\gets \unisrv(Q)$
	\item Form the certified mint transaction $(T,\sigma,\pi)$.
\end{enumerate}
...
Application specific checks follow, e.g., validate the mint authorization based on enclosed mint justification.
...
\begin{enumerate}
\item $(T_{i}, \sigma_{i}, \txhash^i,d_i, \pi_{i})$ is a certified transaction in $(T_{i-1}.D.\pubkey', h^{i-1}_\mathsf{st})$
\item $h^{i}_\mathsf{st}=H(h^{i-1}_\mathsf{st},x_{i-1})$ where $x_{i-1} =T_{i-1}.D.x$.
\end{enumerate}

\section{Security}
Consider a token with the state $S = (\pubkey, \sthash)$. The transfer protocol assures the following properties:
\begin{itemize}
...
\end{itemize}
In this section, we present security proofs for both the no blocking and the no double-spending properties. Security against blocking does not depend on the choice of the commitment scheme and security against double spending assumes computational binding of the commitment scheme.
...
\item $\mathsf{TS}(\prikey,\cdot)$ -- transaction signer that, given as input a transaction $(h,D)$ returns $(\sigma, \txhash, d)$, where
$(\txhash, d)\gets \commit(H(D))$ and
$\sigma \gets \mathsf{S}(\prikey,H(h,\txhash))$
\end{enumerate}
\textbf{Blocking scenario} involves the following steps:
\begin{enumerate}
...
\end{enumerate}
Note that if such a query was made, then the request $Q=(\pubkey,\sthash, \txhash,\sigma)$ to $\mathsf{US}$ will trivially ensure $R[H(\pubkey,h_\mathsf{st})]\neq\bot$, and hence this is excluded by the security condition. \medskip
...
\certver(T,\sigma,\txhash,d,\pinc;\pubkey,h)=\certver(T',\sigma',\txhash',d',\pinc';\pubkey,h)=1
\end{equation}
\end{enumerate}
...
\begin{enumerate}
\item If $R[H(\pubkey, h_\mathsf{st})]=\bot$ and $\mathsf{V}(\pubkey, h_T, \sigma)=1$ then $R[H(\pubkey, h_\mathsf{st})]\gets h_T$
\item Return a proof $\pi$ of the statement $R[H(\pubkey, h_\mathsf{st})]=h_T$
\end{enumerate}
Assume that a user $A$ owns a token in a state $(\pubkey,h_\mathsf{st})$
and a malicious user $B$ that knows a signed (by $A$) transaction $(T,\sigma)$ with $T=(h', D')$. User $B$ may now lock $A$-s token (with the state $(\pubkey,h_\mathsf{st})$) by sending a request $Q=(\pubkey,h_\mathsf{st}, h_T, \sigma)$ to $\mathsf{US}$.
Therefore, such a modification is insecure against blocking.

\subsubsection{$R$ as a Set}

Consider the following modification of $\mathsf{US}$ where $R$ is just a set such that given a request $Q=(\pubkey, \sthash, h_\mathsf{tx}, \sigma)$ proceeds as follows:
\begin{enumerate}
\item If $H(\pubkey, \sthash)\not\in R$ and $\mathsf{V}(\pubkey, H(\sthash, h_\mathsf{tx}), \sigma)=1$ then $R\gets R \cup \{H(\pubkey, \sthash)\}$
\item Return an inclusion proof $\pinc$ of the statement $H(\pubkey, \sthash)\in R$
\end{enumerate}
Assume that a malicious $A$ owns a token in the state $(\pubkey,h_\mathsf{st})$ and prepares two transactions
$T_1=(h_\mathsf{st},D_1), T_2=(h_\mathsf{st},D_2)$ that transfer the token to two different public keys $\pubkey_1$ and $\pubkey_2$. The user $A$ signs $T_1$ by computing $\sigma\gets \sig(\prikey, H(\sthash,H(D_1)))$ and sends a request $Q=(\pubkey,\sthash,H(D_1),\sigma)$ to $\mathsf{US}$.

As $\sigver(\pubkey,H(\sthash,H(D_1)),\sigma)=1$, $\mathsf{US}$ assigns $R\gets R\cup\{H(\pubkey,\sthash)\}$ and returns the inclusion proof $\pinc$ of the statement $H(\pubkey,\sthash)\in R$.

After that, $A$ signs $T_2$ by computing $\sigma'\gets \sig(\prikey, H(\sthash,H(D_2)))$ and composes a certified transaction $(T_2,\sigma',\pinc)$ that double-spends the token.
...
$Q'=(\pubkey',\sthash',\txhash',\sigma')$ with $\sthash'=H(\sthash,x)$ received by $\unisrv$.
...
\item Find $x$ based on the commitment $\txhash=\commitc(D)$. To prevent that, we may assume that the commitment scheme is computationally binding.
...
\end{itemize}
Note that if $\ell$ is large, then $H(\sthash,x)$ with $x\gets \{0,1\}^\ell$ may give very little information about the previous state hash $\sthash$.
For example, an extreme case is that if $\ell=k$ and the function $H(h,\cdot)\colon \{0,1\}^k\rightarrow \{0,1\}^k$ happens to be one-to-one for every $h$ (with most likely does never happen for practical hash functions) then in fact $H(\sthash,x)$ gives no information on $\sthash$  because the equation $H(h,x)=\sthash'$ can be (uniquely) solved for every state hash $h$ that $\unisrv$ has stored or memorized. In practice, there is no need to choose very large $\ell$ as practical security is possible if $\ell$ is much smaller than $k$.
...
For any fixed value of $L=(\sthash, \pubkey', \auxd, a)$, due to perfect hiding, the commitments $\txhash=\commitc(H(\pubkey',x,\auxd))$ and $\txhash'=\commitc(H(\pubkey',x'',\auxd))$ have equal probability distributions. Moreover, by Lemma~\ref{le:outputindependence} (with $g(x)=H(\pubkey',x,\auxd)$), the random variable $x$ and $\txhash=\commitc(H(\pubkey',x,\auxd))$ are independent.
...
\end{enumerate}
\end{itemize}
The computational time overhead function of $A'$ is $\tau(t) = t + t_\mathsf{sm} + t_\mathsf{hash} + t_\mathsf{com}$ and hence, by equation (\ref{eq:losebound}), the function $S_\mathsf{assoc}$ defined by
\[
