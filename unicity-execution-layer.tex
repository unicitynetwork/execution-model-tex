...
For applications requiring persistent public keys as ``addresses'', or non-interactive operation, the parties can create the blinded public key using Diffie-Hellman key exchange. The recipient publishes a persistent public key $P = d \cdot G$. The protocol is shown in Figure~\ref{fig:protocol2}.
...
\underline{Setup:} Recipient publishes persistent public key $P = d \cdot G$
...
$S \leftarrow r \cdot P$ & & \\
$m \leftarrow \mathcal{H}(S)$ & & \\
$P_{tx} \leftarrow P + m \cdot G$ & & \\
$\mathsf{tx} \leftarrow \{P_{tx}, R, \ldots\}$ & & \\
& $\xrightarrow{\hspace{1.5cm} \mathsf{tx} \hspace{1.5cm}}$ & \\[3mm]
& & $S \leftarrow d \cdot R$ \\
& & $m \leftarrow \mathcal{H}(S)$ \\
& & $d_{tx} \leftarrow d + m \bmod n$ \\
& & $\sigma \leftarrow \mathsf{Sign}_{d_{tx}}(\mathsf{tx}_{next})$ \\
\end{tabular}
\end{minipage}
}
\caption{Non-interactive protocol using Diffie-Hellman key exchange to blind addresses. The recipient can derive signing keys for any transaction without prior interaction. We assume honest sender.}
\label{fig:protocol2}
\end{figure}

The sender generates an ephemeral keypair $(r, R = r \cdot G)$ and computes a Diffie-Hellman shared secret $S = r \cdot P$. The blinding mask $m = H(S)$ is used to compute the blinded public key $P' = P + m \cdot G$. The ephemeral public key $R$ is included in $\auxd$. Upon receiving the transaction, the recipient computes the same shared secret $S = d \cdot R$ and derives $d' = d + m \bmod n$.

This construction is insecure because the sender can choose a predictable $r$ or leak it, thereby compromising the transaction unlinkability.
...
\caption{Protocol secure against malicious sender.}
\label{fig:protocol3}
\end{figure}

This is fixed in Protocol shown at Fig.~\ref{fig:protocol3}. We assume that $\mathsf{tx}_{prev}$ is always unique because malicious sender reusing exact $(r, \mathsf{tx}_{prev})$ pair could break unlinkability.
