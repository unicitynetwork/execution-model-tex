...
Predicates generalize the concepts of token ownership and transfer in the Unicity infrastructure, which in the paper \cite{BTLR25} were defined via digital signatures as follows:
\begin{itemize}
...
\section{Unicity Infrastructure with Predicates}

\subsection{Unicity Service}

Like in the paper \cite{BTLR25} we model the Unicity Service as an ideal functionality, where we use \emph{system time} as an additional state variable.
...
\noindent In this section, we present security proofs for all three properties. The proofs of no double-spending and no association are very similar to the proofs in \cite{BTLR25}. The no blocking property
